...
import SkyFoxRobot.modules.sql.chatbot_sql as sql
from SkyFoxRobot.modules.helper_funcs.filters import CustomFilters
from SkyFoxRobot.modules.helper_funcs.chat_status import user_admin, user_admin_no_reply
from SkyFoxRobot import dispatcher, updater, SUPPORT_CHAT
from SkyFoxRobot.modules.log_channel import gloggable
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "SkyFox Chatbot disable by {}.".format(
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "SkyFox Chatbot enable by {}.".format(
                    mention_html(user.id, user.first_name)
                ),
                parse_mode=ParseMode.HTML,
            )
...
def chatbot(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        kukiurl = requests.get(
            "https://www.kukiapi.xyz/api/apikey=KUKIg76Fg4EIo/Natsunagi/@la1yy/message="
            + Message
        )
...
def list_all_chats(update: Update, context: CallbackContext):
    ...
    text = "<b>SkyFoxRobot Enabled Chats</b>\n"
    for chat in chats:
        try:
